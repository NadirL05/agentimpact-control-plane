-- TEST ONLY: immutable original 008 from c08bfa89ec579744f85aff4d3886c2d81d57202c.
-- Never include in an installation or execute against a persistent database.
...
DO $$
BEGIN
  IF to_regclass('mission_approval_bindings') IS NULL
    OR to_regclass('agent_actions') IS NULL
    OR to_regclass('agent_approvals') IS NULL
    OR to_regclass('codex_attempt_metadata') IS NULL
    OR to_regclass('worktree_leases_one_codex_writer_per_repo') IS NULL THEN
...
BEGIN
  ...
  FROM mission_approval_bindings b
  JOIN agent_actions x ON x.id=b.action_id
  JOIN agent_approvals p ON p.action_id=x.id
  WHERE b.mission_id=p_mission_id AND b.attempt_id=p_attempt_id AND b.action_type=p_action_type AND b.payload_hash=p_payload_hash
    ...
    AND p.decided_at >= (SELECT created_at FROM mission_attempts WHERE id=p_attempt_id)
