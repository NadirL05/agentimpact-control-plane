#!/usr/bin/env bash
# Jarvis V1.2 ONE-SHOT Codex canary V3-QUOTA — post multi-gate runtime.
...
REPORT="${REPORT_DIR}/jarvis-v1-2-codex-canary-v3-quota-${STAMP}.txt"
ROLLBACK_DIR="/var/lib/agentimpact/rollback/jarvis-v1-2-canary-v3-quota-${STAMP}"
FIXTURE_SRC="${REPO}/infra/jarvis/canary-fixtures/increment"
FIXTURE_HOST="/var/lib/agentimpact-superset/fixtures/jarvis-v1-2-codex-canary-v3-quota-${STAMP}"
API_BASE="${API_BASE:-http://127.0.0.1:3000}"
ORG_ID="org-jarvis-v12-canary-v3q"
...
BRIDGE_DROPIN="${BRIDGE_DROPIN_DIR}/99-jarvis-canary-v3-quota-temp.conf"
...
verify_live_safe_flags() {
  ...
  # Armed markers must be absent or explicitly 0 after restore
...
cleanup_canary() {
  ...
  FLAGS_RESTORED=1
  # If we never armed runtime (e.g. base compose precheck failed before auth),
  # do not recreate containers or consume further state.
...
log "=== JARVIS V1.2 CODEX CANARY V3-QUOTA ==="
...
# --- HARD preflight: base compose must validate BEFORE auth consumption ---
if ! docker compose -f "${BASE_COMPOSE}" config >/dev/null; then
  log "BASE_COMPOSE_CONFIG=FAIL"
  log "BASE_COMPOSE_PRECHECK=FAIL"
  log "REAL_CODEX_CALLS=0"
  log "REAL_CURSOR_CALLS=0"
  log "NOTE=auth_not_consumed_base_compose_invalid"
  exit 1
fi
log "BASE_COMPOSE_CONFIG=PASS"
log "BASE_COMPOSE_PRECHECK=PASS"
...
# --- Authorization (root one-shot file) — AFTER base compose precheck ---
...
# Jarvis V1.2 Codex canary V3-FINAL — temporary override only.
...
log "TEMP_COMPOSE_OVERRIDE=PASS"
...
QUOTA_TMP="$(mktemp)"
if ! dc exec -T api sh -c 'node --import tsx /opt/agentimpact/app/src/scripts/jarvis-quota-decision.ts codex' >"${QUOTA_TMP}" 2>/dev/null; then
  if ! dc exec -T api sh -c 'cd /opt/agentimpact/app && node --import tsx src/scripts/jarvis-quota-decision.ts codex' >"${QUOTA_TMP}" 2>/dev/null; then
    rm -f "${QUOTA_TMP}"
...
open(path+".env","w").write("\n".join([
  ...
  f"QUOTA_CHECK={'PASS' if d.get('authorizationClass')=='ALLOW' else ('BLOCKED_EXHAUSTED' if d.get('authorizationClass')=='DENY_EXHAUSTED' else 'BLOCKED_UNKNOWN')}",
])+"\n")
PY
...
print(json.dumps({
  ...
  "parameters": {
    ...
    "name": "jarvis-v12-canary-v3-final",
    "branch": "jarvis/v12-codex-canary-v3-final"
  }
}))
PY
...
CANARY_PROMPT="Jarvis V1.2 one-shot Codex canary v3-final. Modify ONLY the file src/increment.js. The function increment(n) must return n + 1. Do not create other files. Do not run shell. Do not push git. Stop when the change is done."
