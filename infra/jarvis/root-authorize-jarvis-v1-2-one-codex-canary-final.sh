#!/usr/bin/env bash
# FINAL root one-shot authorization for Jarvis V1.2 Codex canary armed-v2.
#
# outer-verify-and-run.py always execs: bash <verified_helper> <legacy_tarball>
# This helper accepts 0 or exactly 1 positional (ignored). No caller-controlled
# script path, provider, scope, or TTL. No flags.
#
# Does NOT launch Codex/Cursor. Does NOT modify outer-verify.
# Static probe (tests only): AGENTIMPACT_AUTH_HELPER_STATIC_PROBE=1
#
# Writes (live, as root): /run/agentimpact-jarvis-canary/codex-one-shot.auth
set -euo pipefail

CANONICAL_CANARY=/opt/agentimpact/runner/superset-rpc-bridge/scripts/root-run-jarvis-v1-2-codex-canary-armed-v2.sh
...
# Fail closed on any flag. Only bare positionals (0 or 1) allowed.
POSITIONALS=()
while [[ $# -gt 0 ]]; do
  case "$1" in
    -*)
...
# Discard legacy positional entirely — never use as path, never read, never exec.
if (( ${#POSITIONALS[@]} == 1 )); then
  unset 'POSITIONALS[0]'
  LEGACY_COUNT=1
else
...
# Test-only overrides (STATIC_PROBE only). Never used on live authorize path.
...
# Hardcoded expected SHA only — never SCRIPT_SHA from an alternate path in live mode.
